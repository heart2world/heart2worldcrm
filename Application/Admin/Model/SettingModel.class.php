...
class SettingModel {
	 ...
	 * 业务字段列表
	 */
	public function get_fields_lists($data, $page = 0, $row = 0){
		$model = M('fields');
		$fields = 'id,title,sort,is_open,is_must';
		$map['status'] = 1;
	...
	public function get_fields_info($data){
		...
		$map['id'] = $data['id'];
		$map['status'] = 1;
		$info = $model->field('id,type,title,tips,is_open,is_must,sort,extra')->where($map)->find();
		if($info['extra']){
