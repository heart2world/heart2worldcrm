...
//						$value = getFileUrl($value,'image');
	...
	public function get_info_text($data){
		...
		if($info){
			...
			$cf_data = array_column($cf_data,'content','fields_id');
			//字段名称
			$fields_data = M('fields')->field('id,title,type')->where(array('status'=>1,'is_open'=>1))->order('sort asc,id desc')->select();
			...
			$users_data = M('users')->field('id,name')->select();
			$users_data = array_column($users_data,'name','id');
			$fields_data[] = array('title'=>'客户类型','content'=>$customer_type_data[$info['type']]);
			$fields_data[] = array('title'=>'跟进状态','content'=>$follow_status_data[$info['follow_status']]);
			$fields_data[] = array('title'=>'创建人','content'=>$users_data[$info['users_id']]);
			$fields_data[] = array('title'=>'负责人','content'=>$users_data[$info['person_id']]);
			$fields_data[] = array('title'=>'创建时间','content'=>date('Y-m-d H:i:s',$info['create_time']));
			if($info['remark']){
				$fields_data[] = array('title'=>'备注信息','content'=>$info['remark']);
			}
			$info['fields_data'] = $fields_data;
			$info['person_id'] = $users_data[$info['person_id']];
			//附件
			$file_data = array();
			...
			if($sha1){
				...
				foreach ($file_data as $key => $value) {
					...
					$value['create_time'] = date('Y-m-d H:i',$info['create_time']);
					$value['remark'] = $info['remark'];
					$file_data[$key] = $value;
				}
			}
			$info['file_data'] = $file_data;
	...
	public function get_customer_logs($data){
		...
		$lists = $model->field('id,type,content,create_time')->where($map)->order('create_time desc,id desc')->select();
		if($lists){
	...
	public function add_customer_logs($data){
		$data = set_trim($data);
		if(empty($data['customer_id']) || empty($data['type']) || !in_array($data['type'],[1,2,3,4])){
			return ['msg'=>'数据不存在','status'=>400];
	...
	public function edit($data){
		...
		if(empty($data['users_id'])){
			...
			return ['msg'=>'备注不能为空','status'=>400];
		}elseif(mb_strlen($data['remark'],'utf-8') > 200){
			return ['msg'=>'备注不能超过200个字符','status'=>400];
		}
		//字段数据判断
...
//				if($old_one_data && $new_one_data && $new_one_data != $old_one_data){
//					$log_content[] = array('fields_text'=>$value['id'],'title'=>$value['title'],'content'=>'修改详细内容');
//				}elseif(empty($new_one_data) && $old_one_data){
//					$log_content[] = array('fields_text'=>$value['id'],'title'=>$value['title'],'content'=>'修改详细内容');
//				}elseif(empty($old_one_data) && $new_one_data){
//					$log_content[] = array('fields_text'=>$value['id'],'title'=>$value['title'],'content'=>'新增详细内容');
//				}
			}
